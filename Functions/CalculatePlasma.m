function [sym, res] = CalculatePlasma(sym, res, fixed, phys, settings)
...
sym.V_p =  2 * pi^2 * sym.R_0 * res.kappa * sym.a^2;

res.p = sqrt(16 * fixed.P_e * (phys.k * res.T)^2 / (fixed.eta_e * (phys.E_f + phys.E_Li) * res.V_p * phys.sigma_v));
sym.p = sqrt(16 * fixed.P_e * (phys.k * res.T)^2 / (fixed.eta_e * (phys.E_f + phys.E_Li) * sym.V_p * phys.sigma_v));
res.n = res.p/(2 * phys.k * res.T);
sym.n = sym.p/(2 * phys.k * res.T);

res.tau_e = 3/2 * res.p / (1/4 * res.n^2 * phys.sigma_v * 1/5 * phys.E_f);
sym.tau_e = 3/2 * sym.p / (1/4 * sym.n^2 * phys.sigma_v * 1/5 * phys.E_f);
...
sym.I_p = ((sym.tau_e * (P_alpha * 1e-6)^(0.69)) / (0.145 * phys.H * sym.R_0^(1.39) * sym.a^(0.58) * res.kappa^(0.78) * (sym.n*1e-20)^(0.41) * sym.B_0^(0.15) * phys.A^(0.19)))^(1/0.93)*1e6;
...
for k=1:numel(names)
    try
        res.(names{k}) = double(subs(res.(names{k}), settings.var, res.(char(settings.var))));
    end
end
