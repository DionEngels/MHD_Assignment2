function [res] = CalculatePlasma(res, phys, settings)
...
res.p = sqrt(16 * res.P_e * (phys.k * res.T)^2 / (res.eta_e * (phys.E_f + phys.E_Li) * res.V_p * phys.sigma_v));
...
res.I_p = ((res.tau_e * (P_alpha * 1e-6)^(0.69)) / (0.145 * res.H * res.R_0^(1.39) * res.a^(0.58) * res.kappa^(0.78) * (res.n*1e-20)^(0.41) * res.B_0^(0.15) * phys.A^(0.19)))^(1/0.93)*1e6;
